...
import data from "./HamiltonCircuit.json";

export default class HamiltonCircuit extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            "won": false,
    ...
    evaluateWin(history) {
        if (history.length === data[this.props.match.params.level - 1].v.length + 1 && history[0].v[0] === history[history.length - 1].v[0] && history[0].v[1] === history[history.length - 1].v[1]) {
            this.setState({
                "won": true
            });
        }
